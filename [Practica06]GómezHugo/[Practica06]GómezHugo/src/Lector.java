...
public class Lector {
                ...
                if (aux != null) {
                    if(i==0){
                        ...
                        for(int j = 0; j<arrSep.length; j++){
                            listaVertices.add(new Vertice(arrSep[j].trim()));
                        }
                    }else{
                    String[] sepAristas = aux.split(",");
                    listaAristas.add(new Arista(new Vertice(sepAristas[0].trim()), new Vertice(sepAristas[1].trim())
    ...
    public static void escritura(String direccion, String vertices, ArrayList<Arista> listaAristas){
